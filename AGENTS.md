...
3. Use `working` when running bash commands or doing a long-running task.
4. Use `reading` when focused on reading prose, documentation, or other longer-form content.
5. Use `thinking` when reading files, reviewing code, or otherwise inspecting the project.
6. While thinking/working/reading, every few substantial thoughts or progress steps, update the face to something appropriate.
7. Do not leave the LED on during active work.
8. At a stopping point, always set an appropriate result face and then initiate the LED.
9. At a stopping point, set LED brightness to `0.3` always:
...
11. Once work resumes again after a stopping point, turn the LED off and set a new working face.
